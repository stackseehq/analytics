...
The Proxy ships with trakoo, so it has no extra package of its own. Install the SDK for each server provider you forward to—for example, `posthog-node` for PostHog or `@emitkit/js` for [EmitKit](/docs/providers/emitkit). [Pirsch](/docs/providers/pirsch) needs no additional package.
